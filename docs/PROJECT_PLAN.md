...
1. Create one English game from a versioned ruleset and lexicon manifest.
...
5. Watch the game in the web UI and replay it from stored events.
6. Produce a result containing scores, timings, agent manifests, and hashes of
   every reproducibility input.
...
- Treat racks and tile draws as seat-private information.
...
- [ ] Add golden games, property tests, and a small licensed test lexicon.
...
ruleset, lexicon, event stream, and RNG inputs.

## Phase 2: application service and persistence

- [ ] Define application commands and seat-aware queries around the engine.
- [ ] Add PostgreSQL migrations for games, seats, events, snapshots, rulesets,
  lexicons, tournaments, matches, and agent runs.
- [ ] Append events and update snapshots in one optimistic-concurrency
  transaction using expected game versions.
- [ ] Separate public, per-seat private, and administrator projections.
...
- [ ] Build a PostgreSQL-backed job claim/lease loop before adding a queue.
...
- [ ] Build a lexicon compiler to a compact memory-mapped exact/prefix format.
- [ ] Define manifests containing provenance, locale, version, license,
  normalization, checksum, and compatibility metadata.
- [ ] Publish clearly licensed community packs and documented import workflows
  for operator-supplied tournament dictionaries.
