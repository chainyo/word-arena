...
pub static MIGRATOR: LazyLock<Migrator> = LazyLock::new(|| {
    Migrator::with_migrations(vec![
        ...
        Migration::new(
            ...
            "durable jobs".into(),
            MigrationType::Simple,
            include_str!("../migrations/0009_jobs.sql").into_sql_str(),
            false,
        ),
        Migration::new(
            10,
            "scheduler controls".into(),
            MigrationType::Simple,
        ...
        Migration::new(
            14,
            "four player seats".into(),
            MigrationType::Simple,
            include_str!("../migrations/0014_four_player_seats.sql").into_sql_str(),
            true,
        ),
    ])
});
...
mod tests {
    ...
    async fn clean_install_is_idempotent_and_applies_migrations_in_order() {
        ...
        for required in [
            ...
            "tournament_series",
            "tournament_worker_controls",
            "tournaments",
            "terminal_match_results",
            "turn_deadlines",
        ] {
            assert!(
